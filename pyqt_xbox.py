...
class Worker(QObject):
    button_change = pyqtSignal(int, int)
    axis_change = pyqtSignal(str, float)
    confirm = pyqtSignal()
    cancel = pyqtSignal()
    left = pyqtSignal()
    right = pyqtSignal()
    ...
    def __init__(self, vibrations: bool):
    ...
    def run(self):
        ...
        self.info.n_devices = len(joysticks)
        self.info.devices = device_numbers

        if not joysticks:
            sys.exit(0)

        j = joysticks[0]
        self.info.device = j.device_number

        battery = j.get_battery_information()
        self.info.battery = battery

        @j.event
        def on_button(button, pressed):
            # send general button signal
            self.button_change.emit(button, pressed)

            # send additional signal
            if not pressed:
                if button == 13:
                    self.confirm.emit()
                elif button == 14:
        ...
        def on_axis(axis, value):
            ...
            self.axis_change.emit(axis, value)
...
class QtXBoxInput:
    def __init__(self, vibrations: bool = True):
        self.running = False

        self.thread = QThread()
        self.worker = Worker(vibrations)
        self.worker.moveToThread(self.thread)
        self.thread.started.connect(self.worker.run)

        self.button_change = self.worker.button_change
        self.axis_change = self.worker.axis_change
        self.confirm = self.worker.confirm
        self.cancel = self.worker.cancel
        self.left = self.worker.left
        self.right = self.worker.right
        self.up = self.worker.up
        self.down = self.worker.down
